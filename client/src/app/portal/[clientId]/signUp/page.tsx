...
export default function ClientSignupPage({ params, searchParams }: { params: Promise<{ clientId: string }>; searchParams: Promise<{ token?: string }> }) {
  ...
  const select = (field: StringField, label: string, options: { label: string; value: string }[]) => {
    const required = !optionalFields.has(field);
  ...
  async function submit(e: React.FormEvent) {
    ...
    for (const field of stringFields) {
      if (!optionalFields.has(field) && !String(form[field] ?? "").trim()) {
        missing.push(requiredLabels[field] || field);
      }
    }
    ...
    if (healthInsuranceFiles.length === 0) {
      missing.push(requiredLabels.healthInsuranceCards || "Health Insurance Cards");
    }
