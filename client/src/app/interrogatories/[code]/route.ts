...
// Redirect short interrogatory URL to the existing secure portal URL.
export async function GET(req: NextRequest, context: { params: Promise<{ code: string }> }) {
    try {
        ...
        const url = `${baseUrl}/portal/${encodeURIComponent(client.clientId)}/interrogatories?token=${encodeURIComponent(client.clientAccessToken)}`;
        return NextResponse.redirect(url);
    } catch (error) {
        console.error("INTERROGATORIES SHORT LINK ERROR", error);
