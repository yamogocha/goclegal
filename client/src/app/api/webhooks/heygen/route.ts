...
async function processHeyGenWebhook(body: any): Promise<void> {
    ...
    let completed = false;
    try {
        console.log("[HEYGEN] Background processing started:", JSON.stringify(body, null, 2));
        if (body?.event_type !== "avatar_video.success") return;
        ...
        if (ad.status === "publishing") return;
        ...
        // Existing storyboard/timeline pipeline.
        ...
        // Download rendered video for YouTube.
        ...
        if (!instagramReelId) {
            for (let i = 0; i < 5; i++) {
                try {
                    instagramReelId = await publishInstagramReel({
                        igUserId: process.env.IG_USER_ID!,
                        userAccessToken: process.env.FB_USER_ACCESS_TOKEN!,
                        reelUrl,
                        caption,
        ...
        if (!facebookReelId) {
            facebookReelId = await publishFacebookReel({
                fbPageId: process.env.FB_PAGE_ID!,
                pageAccessToken: process.env.FB_PAGE_ACCESS_TOKEN!,
                reelUrl,
                caption,
        ...
        if (!youtubeVideoId) {
            youtubeVideoId = await uploadYoutubeVideo({
                videoBuffer: renderedBuffer,
                title: ad.title,
        ...
        // Complete job.
        await serverClient.patch(ad._id).set({
            status: "completed",
        ...
        }).catch(err => console.error("[SLACK] Completion notification failed:", err));
    } catch (err) {
        if (tempAvatarPath) await fs.unlink(tempAvatarPath).catch(() => { });
        if (!completed) {
            ...
            if (ad?._id) await serverClient.patch(ad._id).set({
                status: "pending",
                error: getErrorMessage(err),
            }).commit().catch(cleanupErr => console.error("[SANITY] Failed resetting weeklyAd:", cleanupErr));
        }
...
export async function POST(req: Request) {
    ...
    after(async () => {
        await processHeyGenWebhook(body);
    });
