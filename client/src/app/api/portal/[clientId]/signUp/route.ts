...
const fileFields = ["driverLicense", "healthInsuranceCards", "medicalRecords", "declarationPage"] as const;
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ clientId: string }> }) {
    try {
        ...
        // Append multiple Sanity file assets without replacing existing uploads.
        for (const field of arrayFileFields) {
            ...
            patchData[field] = [...(existing[field] || []), ...assets];
        }
        ...
        if (declarationPage instanceof File && declarationPage.size > 0) {
            const buffer = Buffer.from(await declarationPage.arrayBuffer());
            const asset = await serverClient.assets.upload("file", buffer, {
