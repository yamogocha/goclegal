...
export default function WelcomeCompnent(welcomePage: WelcomePage) {
  const { image, title, description, buttonText, phoneNumber } = welcomePage
    ...
    return (
        <main id="home" className="scroll-mt-20 relative overflow-hidden">
          ...
          <div className="relative z-5 max-w-[1200px] m-auto w-full h-[calc(100vh-119px)] p-5 flex flex-col justify-center items-start">          
              <MotionWrapper>
                <h1 className="w-full lg:w-[800px] text-[48px] lg:text-[64px] leading-tight lg:leading-normal font-bold pb-6 lg:pb-12">{title}</h1>
              </MotionWrapper>
              ...
              <div className="flex flex-col lg:flex-row items-center justify-end w-full">
                  <MotionWrapper type={Motions.FADEUP}>
                    <p className="font-montserrat font-medium w-full lg:w-[600px] text-[18px] lg:text-[24px] pb-6">{description}</p>
                  </MotionWrapper>
                  <Link href="tel:+15108460928" className="font-montserrat mb-4 w-full lg:w-[250px] text-white text-center text-[18px] lg:text-[20px] font-bold p-4 bg-[#00305b] hover:bg-[#0f4c85] transition duration-300 ease-out">{buttonText}</Link>
              </div>
          </div>
        </main>
    )
