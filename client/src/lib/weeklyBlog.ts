...
export async function generateWeeklyBlog() {
    try {
        ...
        const blogResp = await openai.responses.create({
            ...
            text: { format: { type: "json_schema", name: "weekly_blog", schema: weeklyPostSchema } },
            input: postRespInput
        });
        ...
        const uploadedImage = await serverClient.assets.upload("image", imageBuffer, { filename: `${slug}.jpeg`, contentType: "image/jpeg" });
