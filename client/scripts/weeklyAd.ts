...
async function main() {
  ...
  try {
    ...
    if (!result) {
      throw new Error("Weekly ad returned empty result.");
    }

    const failed = !result.ok || result.igError || result.youtubeError || result.gbpError;
    console.log("::group::Weekly Ad Result");
    console.log(JSON.stringify({ ok: !failed, durationMs: Date.now() - start, result }, null, 2));
    console.log("::endgroup::");
    if (failed) {
      console.error("[WEEKLY AD FAILED]", JSON.stringify(result, null, 2));
      process.exit(1);
    }
